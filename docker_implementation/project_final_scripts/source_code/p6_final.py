#--------- p6 - Number of users per location ---------------------------#
...
logging.basicConfig(filename=config.proj_path+'/log_for_demo/p6/p6.log',level=logging.DEBUG)
...
if __name__ == '__main__':
    ...
    r0  = pool(delayed(forcomb)(l,user_count_df,qt1) for l in l_list) 
    ...
    if(real_flag==1):
        prophet_analysis_df.to_csv(config.proj_path+'/p6/user_count_analysis_per_location_data.csv',index=False)
        prophet_df.to_csv(config.proj_path+'/p6/user_count_evaluation_per_location_data.csv',index=False)
    
        prophet_future_df.to_csv(config.proj_path+'/p6/user_count_forecast_per_location_data.csv',index=False)
    
        user_count_df.to_csv(config.proj_path+'/p6/user_count_per_location_dataset.csv',index=False)
    ...
    if(real_flag==1):
        prophet_future_df.to_sql(con=engine, name='forecast_p6', if_exists='replace', index=False )
        prophet_df.to_sql(con=engine, name='eval_p6', if_exists='replace', index=False )
        user_count_df.to_sql(con=engine, name='data_p6', if_exists='replace', index=False )
    ...
    if(config.override_flag == 1):
        prophet_analysis_df.to_sql(con=engine, name='analyse_p6', if_exists='replace',index=False)
    else:
        prophet_analysis_df.to_sql(con=engine, name='analyse_p6', if_exists='append',index=False)

    print(total_time)
    run_time_table = pd.DataFrame({'report':'p6','run_time':total_real,'run_date':datetime.now().date()},index=[6])
