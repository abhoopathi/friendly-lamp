...
if __name__ == '__main__':
    ...
    if(real_flag==1):
        prophet_future_df.to_sql(con=engine, name='forecast_p7', if_exists='replace', index=False )
        prophet_df.to_sql(con=engine, name='eval_p7', if_exists='replace', index=False )
        bw_df.to_sql(con=engine, name='data_p7', if_exists='replace', index=False )
    ...
    print(total_time)
    run_time_table = pd.DataFrame({'report':'p7','run_time':total_real,'run_date':datetime.now().date()},index=[7])
