...
if not os.path.exists(newpath):
    os.makedirs(newpath)
newpath = config.proj_path+'/log_for_demo/p9' 
if not os.path.exists(newpath):
    os.makedirs(newpath)

if(real_flag==1):    
    newpath = config.proj_path+'/p9' 
    if not os.path.exists(newpath):
        os.makedirs(newpath)
#for handler in logging.root.handlers[:]:
#    logging.root.removeHandler(handler)
    
logging.basicConfig(filename=config.proj_path+'/log_for_demo/p9/p9.log',level=logging.DEBUG)
...
if __name__ == '__main__':
    ...
    df1 = sqlContext.sql('select sum(byte_count) as byte_count_sum, time_stamp, source from dummy group by source, time_stamp')
    ...
    r0  = pool(delayed(forsource)(s,bw_df,qt1) for s in s_list) 
    ...
    if(real_flag==1):
        prophet_analysis_df.to_csv(config.proj_path+'/p9/bw_analysis_per_source_data.csv',index=False)
        prophet_df.to_csv(config.proj_path+'/p9/bw_evaluation_per_source_data.csv',index=False)
        prophet_future_df.to_csv(config.proj_path+'/p9/bw_forecast_per_source_data.csv',index=False)
        bw_df.to_csv(config.proj_path+'/p9/bw_per_source_dataset.csv',index=False)
    ...
    if(real_flag==1):
        prophet_future_df.to_sql(con=engine, name='forecast_p9', if_exists='replace', index=False )
        prophet_df.to_sql(con=engine, name='eval_p9', if_exists='replace', index=False )
        bw_df.to_sql(con=engine, name='data_p9', if_exists='replace', index=False )
    ...
    if(config.override_flag == 1):
        prophet_analysis_df.to_sql(con=engine, name='analyse_p9', if_exists='replace',index=False)
    else:
        prophet_analysis_df.to_sql(con=engine, name='analyse_p9', if_exists='append',index=False)

    print(total_time)
    run_time_table = pd.DataFrame({'report':'p9','run_time':total_real,'run_date':datetime.now().date()},index=[9])
