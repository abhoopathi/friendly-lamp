...
def create_prophet_m(app_name,z1,cpu_perc_list,delay=48):
    ...
    #-- Realtime prediction --##
    #model 
    model_r = Prophet(yearly_seasonality=False,changepoint_prior_scale=.2,seasonality_prior_scale=0.2)
    model_r.fit(full_df)
    ...
    #model 
    model = Prophet(yearly_seasonality=False,changepoint_prior_scale=.2,seasonality_prior_scale=0.2)
    model.fit(train_df)
    ...
    print('predicted df : ',pred)
    print('merged df length :',len(pred_df))
...
def forcomb(s,df,ftime1,cpu_perc_list):
    ...
    df2 = pd.DataFrame(df2.groupby(by='date').app_count.max())
    ...
    if(len(df2)>config.limit):
        ...
        prophet_analysis_df,ew_model,ew_forcast,prophet_df,prophet_future_df =(create_prophet_m(s,df2,cpu_perc_list,config.delay_48))
...
def comb_creation(apps,cpu_perc_list):
    ...
    df_t = sqlContext.sql('select count(*) as count, source   from dummy group by source')
...
#def main(day):
    ...
    logging.basicConfig(filename=config.proj_path+'/log_for_demo/p8/p8_48hrs.log',level=logging.INFO)
    ...
    prophet_future_df = pd.DataFrame()
    prophet_analysis_df = pd.DataFrame()
    app_count_full_df = pd.DataFrame()
    ...
    # data cleaning
    app_count_df=df_t.toPandas()

    s_array = app_count_df.source.unique()
    #s_array = rdf.source.unique()

    app_count_df = app_count_df.sort_values(by='app_count',ascending=True)       
    ...
    #app_count_df.to_csv('p3/app_count_per_app_per_source_dataset.csv',index=False)
    ...
    r0  = pool(delayed(forcomb)(s,app_count_df,qt1,cpu_perc_list) for s in s_array) 
    ...
    for i in range(0,len(r0)):
        ...
        app_count_full_df = app_count_full_df.append(r0[i][3])
    ...
    if(real_flag==1):
        ...
        prophet_df.to_sql(con=engine, name='valid_nofapps_per_source_48hrs', if_exists='replace', index=False )
        app_count_full_df.to_sql(con=engine, name='data_nofapps_per_source_48hrs', if_exists='replace', index=False )

    ## checking whether forecast and anlyse table avilable or not
    with conn.cursor() as cursor:
        stmt = "SHOW TABLES LIKE 'forecast_nofapps_per_source_48hrs'"
        cursor.execute(stmt)
    ...
    with conn.cursor() as cursor:
        stmt = "SHOW TABLES LIKE 'analyse_nofapps_per_source_48hrs'"
        cursor.execute(stmt)
        result = cursor.fetchone()
    if result:
    ...
    if(table_present_forecast_flag ==1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source_48hrs add partition (partition new values in ('"+str(this_day)+"'))" 
            cursor.execute(sql)
    ...
    if((real_flag ==1) & (config.override_flag_forecast==1)):
        prophet_future_df.to_sql(con=engine, name='forecast_nofapps_per_source_48hrs', if_exists='replace', index=False )
        table_present_forecast_flag = 0
    elif((real_flag ==1) & (config.override_flag_forecast==0)):
        prophet_future_df.to_sql(con=engine, name='forecast_nofapps_per_source_48hrs', if_exists='append', index=False )
    ...
    if(table_present_analyse_flag == 1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = " alter table analyse_nofapps_per_source_48hrs remove partitioning" 
            cursor.execute(sql)
    ...
    if(config.override_flag_analysis == 1):
        prophet_analysis_df.to_sql(con=engine, name='analyse_nofapps_per_source_48hrs', if_exists='replace',index=False)
        table_present_analyse_flag = 0
    else:
        prophet_analysis_df.to_sql(con=engine, name='analyse_nofapps_per_source_48hrs', if_exists='append',index=False)
    ...
    if(table_present_analyse_flag == 1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "select distinct run_date as run_date from analyse_nofapps_per_source_48hrs " 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE analyse_nofapps_per_source_48hrs PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ("+dt_str+") );" 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE analyse_nofapps_per_source_48hrs PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    ...
    if(table_present_forecast_flag ==1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source_48hrs truncate partition latest" 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source_48hrs drop partition latest" 
            cursor.execute(sql)

        ## to make new as 'latest' partition the inputed table 
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE forecast_nofapps_per_source_48hrs REORGANIZE PARTITION new INTO ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    else:
        # adding the partition
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE forecast_nofapps_per_source_48hrs PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    ...
    run_time_table = pd.DataFrame({'report':'p3(nofapps_per_source)','run_time':total_real,'run_date':this_day},index=[3])
    run_time_table.to_sql(con=engine, name='run_time_table_agg_48hrs', if_exists='append',index=False)
