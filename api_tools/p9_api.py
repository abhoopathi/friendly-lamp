class p9:
    ...
    def create_prophet_m(self,source_name,z1,delay):
    ...
    def get_latest_data(self):
        ...
        # data cleaning
        self.p9_df=df1.toPandas()

        self.p9_df = self.p9_df[self.p9_df.byte_count_sum>0]
        self.p9_df = self.p9_df.sort_values(by='byte_count_sum')
        self.p9_df['bw'] = self.p9_df['byte_count_sum']/(8*3600)

        dates = self.p9_df.time_stamp#.astype(int)
        dates_outlook =pd.to_datetime(pd.Series(dates),unit='ms')
        self.p9_df.index = dates_outlook   
        self.p9_df['date'] = self.p9_df.index.date
        self.p9_df = self.p9_df.sort_values(by='time_stamp')
    ...
    def forcomb(self,s):
        ...
        p9_df = self.get_latest_data()

        temp2 = self.p9_df[self.p9_df.source==s]
