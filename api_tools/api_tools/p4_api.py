class p4:
    ...
    def forcomb(self,t,s):
        ...
        q1 = datetime.now()
        df1 = self.df[(self.df.target_address == t) & (self.df.source == s)]

        df_t = df1.registerTempTable('dummy')
        df_t = self.sqlContext.sql('select sum(byte_count) as byte_count_sum , time_stamp , source, target_address from dummy  group by source, target_address, time_stamp')
        #df_t = df_t[df_t.byte_count_sum!=0]

        df2 = df_t.toPandas()

        df2 = df2[df2.byte_count_sum!=0]
        df2['bw'] = df2['byte_count_sum']/(8*3600)
        ...
        if(len(df2)>1400):
            ...
            prophet_analysis_df,ew_model,ew_forcast,prophet_df,prophet_future_df =(self.create_prophet_m(s,df2,24))
            t2 = datetime.now()
